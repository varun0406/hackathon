 ...
 # if i>=len(self.data)-20:
        ...
        for i in range(1,10000):
            ...
            for kk in range(i,i+20):
                ...
                peak1=min(dac[start:mid])
                peak2=min(dac[mid:end])
                ...
                dist1=in1-start
                dist2=end-in2
                if dac[in1]<dac[start] and dac[in2]<dac[end] and (dist1>2 and dist1<5) and (dist2>3 ) and  a<=10  :
                    ...
                    if (((in1-start)<5 and ((in1-start)>=2) and (end-in2)<=9)) and abs(float(peak1)-float(peak2))<101:
                        ...
                        if (End_time-strt_time)<20 and (End_time-strt_time)>10 and abs(float(dac[start])-float(dac[end]))<100 and abs(float(dac[start])-float(dac[low_ind]))<50 and abs(float(dac[low_ind])-float(dac[end]))<50:
                            lst.append(['',self.data[strt_time][5],self.data[End_time][5],"Double Bottom"])
        return lst                 
"""  #print(in1,"&&",in2)
                    strt_time=i
                    End_time=i+20
